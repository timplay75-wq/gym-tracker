...
## Как устроен поток входа

Токен больше не приходит в query-строке редиректа — она оседает в истории
браузера, в заголовке `Referer` и в логах прокси. Сейчас поток такой:

1. Фронтенд генерирует `nonce`, кладёт его в `sessionStorage` и уходит на
   `GET /api/oauth/<provider>?nonce=<nonce>`.
2. Сервер генерирует `state`, кладёт `state.nonce` в httpOnly-cookie и
   редиректит к провайдеру.
3. На колбэке сервер сверяет `state` из query с cookie. Несовпадение —
   отказ (защита от подсунутого злоумышленником authorization code).
4. Сервер редиректит на `FRONTEND_URL/oauth-callback?code=<одноразовый код>`.
5. Фронтенд шлёт `POST /api/oauth/exchange` с `{ code, nonce }` и получает JWT.
   Код одноразовый, живёт 2 минуты и действителен только с тем `nonce`,
   который сгенерировал этот же браузер.

**Привязка к существующему аккаунту** происходит только если провайдер
подтвердил email (`verified_email` у Google, `primary && verified` у GitHub).
Microsoft Graph признак подтверждения не отдаёт, поэтому вход через Microsoft
к уже существующему аккаунту с тем же email не привязывается — пользователь
получит подсказку войти паролем.

> Одноразовые коды хранятся в памяти процесса. При переходе на несколько
> инстансов сюда нужен общий стор (Redis/Mongo), иначе обмен кода будет
> попадать не на тот процесс.
