...
- **Один промт = одна фича** — не пытаться реализовать несколько вещей сразу
- **Большую задачу нужно разбивать на отдельные промты** — каждый делает одно, но хорошо
- **Коммит только после подтверждения** — ждать пока пользователь проверит и скажет что работает
- Подробнее см. [RULES.md](./RULES.md)
